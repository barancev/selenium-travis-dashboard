<template>
  <div id="sidebar2" class="sidebar">
    <h2>Jobs</h2>
    <ul id="sidebar-jobs">
      <li v-for="job in jobs" :key="job.id" :class="[{ active: job.id === currentJob.id }, job.state]" @click="selectJob(job.id)" :title="job.env">
        <state-icon :target="job"></state-icon>{{job.number}}
      </li>
    </ul>
  </div>
</template>
